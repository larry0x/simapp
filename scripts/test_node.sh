#!/bin/bash
# Run this script to quickly install, setup, and run the current version of simapp without docker.
#
# Example:
# CHAIN_ID="local-1" HOME_DIR="~/.simapp1" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
...
from_scratch () {
  ...
  # modify genesis values
  update_test_genesis '.consensus_params["block"]["max_gas"]="100000000"'
  update_test_genesis '.app_state["gov"]["voting_params"]["voting_period"]="15s"'  
  
  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="token"'  
  update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.050000000000000000"'    
  update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.010000000000000000"' # sdk 46+
  ...
  BINARY genesis add-genesis-account $KEY 10000000token,10000000utest --keyring-backend $KEYRING
  BINARY genesis add-genesis-account $KEY2 1000000token,10000000utest --keyring-backend $KEYRING
